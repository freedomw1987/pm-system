...
const DEFAULT_PERMISSIONS = [
  ...
  { key: 'tasks.view_all', name: '查看全部任務', category: '任務管理' },
  ...
  { key: 'bugs.view_all', name: '查看全部缺陷', category: '缺陷管理' },
  ...
  { key: 'worklogs.view_all', name: '查看全部工時', category: '工時管理' },
  { key: 'worklogs.create', name: '工時創建', category: '工時管理' },
  { key: 'worklogs.edit', name: '工時編輯', category: '工時管理' },
  { key: 'worklogs.edit_all', name: '編輯全部工時', category: '工時管理' },
  { key: 'worklogs.delete', name: '工時刪除', category: '工時管理' },
  { key: 'worklogs.delete_all', name: '刪除全部工時', category: '工時管理' },
  ...
  { key: 'users.view_all', name: '查看全部用戶', category: '用戶管理' },
...
function requireAdminOrPermission(user: { role?: string } | null | undefined, set: any, permission: string) {
  ...
  if (!hasPermission(user, permission) && user.role !== 'admin') {
    set.status = 403
    return { error: { code: 'FORBIDDEN', message: `Permission denied: '${permission}' or admin role required` } }
  }
  return null
}
...
const roleRoutes = new Elysia({ prefix: '/roles' })
  .get('/permissions', async ({ set, user }) => {
    // Allow any authenticated user to view permissions list
    if (!user) {
      set.status = 401
      return { error: { code: 'UNAUTHORIZED', message: 'Login required' } }
    }
  ...
  .get('/', async ({ set, user }) => {
    const forbidden = requireAdminOrPermission(user, set, 'roles.view')
  ...
  .post(
    '/',
    async ({ body, set, user }) => {
      const forbidden = requireAdminOrPermission(user, set, 'roles.create')
      if (forbidden) return forbidden
  ...
  .put(
    ...
    async ({ params, body, set, user }) => {
      const forbidden = requireAdminOrPermission(user, set, 'roles.edit')
      if (forbidden) return forbidden
  ...
  .delete('/:id', async ({ params, set, user }) => {
    const forbidden = requireAdminOrPermission(user, set, 'roles.delete')
    if (forbidden) return forbidden
