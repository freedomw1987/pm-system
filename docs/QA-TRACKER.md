...
> **Status**: 🟢 2026-06-09 — Sprint 9 收工,sub-list pagination + Reports stats consistency(WorkLogs ↔ Reports 對齊)
> **Update**: 2026-06-09 Sprint 9 closure — sub-list pagination for ProjectDetailPage(3 tabs)/RequirementDetailPage(2 sub-lists)/UsersPage;成本/進度報告用 worklogs 嘅 `where.OR` pattern 修正,統計同 WorkLogs page 一致
...
| 指標 | 數值 (2026-06-09 Sprint 9 結算 — Sub-list pagination + Reports stats consistency) |
|------|------|
| US 總數 | 50+ |
...
| P0 US PASS-UNIT only | **22** (Sprint 9: +US-11.1) |
| P0 US PASS-INT only | **3** (US-8.1, 8.2, 9.3 — Sprint 3 closure) |
| P0 US DEFERRED | **0** 🟢 |
| P0 US NONE | **0** 🟢 |
| P1+ US | 大部分 NONE (low priority) |
| Unit tests 總數 | **549 pass** (Sprint 9: 516 → 549,+33 — projects 5 + users 14 + reports 14 純 helper derive) |
