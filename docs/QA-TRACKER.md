...
| P0 US PASS-UNIT only | **23** (Sprint 10: +US-6.4) |
| P0 US PASS-INT only | **3** (US-8.1, 8.2, 9.3 — Sprint 3 closure) |
| P0 US DEFERRED | **0** 🟢 |
| P0 US NONE | **0** 🟢 |
| P1+ US | 大部分 NONE (low priority) |
| Unit tests 總數 | **558 pass** (Sprint 10: 549 → 558,+9 — worklogs filter RBAC derive 9 cases) |
...
| 2026-06-10 | **Sprint 10 in progress — P0 remaining US test push**:US-6.4(worklogs filter + RBAC gate)由 NONE → PASS-UNIT(9 個 derive test,non-admin 強制 userId + admin departmentId);next US-3.5 / 4.4 / 2.4 / 2.3 / 4.5 |
